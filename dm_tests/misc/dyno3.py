            ...
            match key:
                ...
                case "get_motor_state":
                    if dyno_state.motor_state:
                        writer.write(b"motor_state on\n")
                    else:
                        writer.write(b"motor_state off\n")
...
async def write_hall_effect():
    global reader, writer, hall_effect_ctr
    while True:
...
async def write_psm():
    ...
    while True:
        ...
        writer.write(out.encode())
        print(out)
        await asyncio.sleep(psm_period)

async def hall_effect():
    print("HF")
...
async def main():
    await open_ser("/dev/ttyLP2", 115200)
