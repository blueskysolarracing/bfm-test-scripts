...
class CUSTOM_GPIO_A0:
    def __init__(self, mcp, cs, port, register):
        self.mcp = mcp
        self.port = port
    ...
    def write(self, val):
        self.cs.write(False)
        self.mcp.write_bit(self.port, self.register, 7, val)
        self.cs.write(True)

class CUSTOM_GPIO_RESET:
    def __init__(self, mcp, cs, port, register):
        self.mcp = mcp
        self.port = port
        self.register = register
    ...
    def write(self, val):
        self.cs.write(False)
        self.mcp.write_bit(self.port, self.register, 6, val)
        self.cs.write(True)
...
class NHDC12864A1ZFSWFBWHTT:
    ...
    a0_pin: CUSTOM_GPIO_A0
    '''The mode select pin for the display device.'''
    reset_pin: CUSTOM_GPIO_RESET
...
def read(port, reg, label):
    cs_expander_gpio.write(False)
    l = mcp.read_register(port, reg)
    cs_expander_gpio.write(True)
    print(f'{label}: {l}')

def write_bit(port, reg, bit, val):
    cs_expander_gpio.write(False)
    l = mcp.write_bit(port, reg, bit, val)
    cs_expander_gpio.write(True)
    #print(f'the bit: {l}')

def read_bit(port, reg, bit):
    cs_expander_gpio.write(False)
    l = mcp.read_bit(port, reg, bit)    
    cs_expander_gpio.write(True)
    print(f'the bit: {l}')
...
spi = SPI('/dev/spidev0.0', 0, 1e5)
mcp = MCP23S17(null_gpio, null_gpio, null_gpio, spi)

cs_display_gpio.write(True)
cs_expander_gpio.write(True)

read(Port.PORTA, Register.IODIR,    "A IODIR")
read(Port.PORTA, Register.IPOL,     "A IPOL")
read(Port.PORTA, Register.GPINTEN,  "A GPINTEN")
read(Port.PORTA, Register.DEFVAL,   "A DEFVAL")
read(Port.PORTA, Register.INTCON,   "A INTCON")
read(Port.PORTA, Register.IOCON,    "A IOCON")
read(Port.PORTA, Register.GPPU,     "A GPPU")
read(Port.PORTA, Register.INTF,     "A INTF")
read(Port.PORTA, Register.INTCAP,   "A INTCAP")
read(Port.PORTA, Register.OLAT,     "A OLAT")
#
read(Port.PORTB, Register.IODIR,    "B IODIR")
read(Port.PORTB, Register.IPOL,     "B IPOL")
read(Port.PORTB, Register.GPINTEN,  "B GPINTEN")
read(Port.PORTB, Register.DEFVAL,   "B DEFVAL")
read(Port.PORTB, Register.INTCON,   "B INTCON")
read(Port.PORTB, Register.IOCON,    "B IOCON")
read(Port.PORTB, Register.GPPU,     "B GPPU")
read(Port.PORTB, Register.INTF,     "B INTF")
read(Port.PORTB, Register.INTCAP,   "B INTCAP")
read(Port.PORTB, Register.OLAT,     "B OLAT")
print("\n")

read(Port.PORTA, Register.GPIO,     "A GPIO")
read(Port.PORTB, Register.GPIO,     "B GPIO")

#cs_display_gpio.write(True)
#cs_expander_gpio.write(False)
#mcp.write_register(Port.PORTA, Register.IODIR, [0b00000000])
#mcp.write_register(Port.PORTB, Register.IODIR, [0b00111111])
#cs_expander_gpio.write(True)

'''
write_bit(Port.PORTA, Register.IODIR, 3, 0)
write_bit(Port.PORTA, Register.IODIR, 4, 0)

while True:
while True:
    cs_expander_gpio.write(False)
    #mcp.write_register(Port.PORTB, Register.GPIO, [0b11000000])
    mcp.write_register(Port.PORTA, Register.GPIO, [0b00011000])
    cs_expander_gpio.write(True)
    sleep(1)

    cs_expander_gpio.write(False)
    #mcp.write_register(Port.PORTB, Register.GPIO, [0b00000000])
    mcp.write_register(Port.PORTA, Register.GPIO, [0b00000000])
    cs_expander_gpio.write(True)
    sleep(1)
'''

print("RESET")
write_bit(Port.PORTB, Register.GPIO, 6, True)
sleep(0.1)
read_bit(Port.PORTB, Register.GPIO, 6)
sleep(0.1)
write_bit(Port.PORTB, Register.GPIO, 6, False)
sleep(0.1)
read_bit(Port.PORTB, Register.GPIO, 6)
sleep(0.1)

print("A0")
write_bit(Port.PORTB, Register.GPIO, 7, True)
sleep(0.1)
read_bit(Port.PORTB, Register.GPIO, 7)
sleep(0.1)
write_bit(Port.PORTB, Register.GPIO, 7, False)
sleep(0.1)
read_bit(Port.PORTB, Register.GPIO, 7)
sleep(0.1)

A0 = CUSTOM_GPIO_A0(mcp, cs_expander_gpio, Port.PORTB, Register.GPIO)
#A0 = GPIO('/dev/gpiochip1', 10, 'out')
NOT_RESET = CUSTOM_GPIO_RESET(mcp, cs_expander_gpio, Port.PORTB, Register.GPIO)

'''
while True:
    A0.write(True)
    NOT_RESET.write(True)
    print(True)
    sleep(1)
    A0.write(False)
    NOT_RESET.write(False)
    print(False)
    sleep(1)
'''

driver = NHDC12864A1ZFSWFBWHTT(SPI('/dev/spidev0.0', 3, 1e5), A0, NOT_RESET, cs_display_gpio)
driver.clear_screen()
driver.draw_rect(0, 0, 127, 63)

driver.set_font("dejavusans.ttf")
driver.set_size(8, 14)
driver.draw_word("Welcome to Blue Sky solar racing! 12345678910", 2, 2)
driver.set_size(16, 16)
driver.draw_word("@#$%*^", 1, int(driver.HEIGHT*0.7))
driver.display()
print("DONE")
while True:
    read(Port.PORTA, Register.GPIO, 7)
    sleep(0.5)
    read(Port.PORTB, Register.GPIO, 7)
    sleep(0.5)
