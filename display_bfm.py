...
class CUSTOM_GPIO_A0:
    def __init__(self, mcp, cs, port, register, direction):
        self.mcp = mcp
        self.port = port
    ...
    def write(self, val):
        self.cs.write(False)
        self.mcp.write_register(self.port, self.register, [0b01000000 + (val << 7)])
        self.cs.write(True)
...
class CUSTOM_GPIO_RESET:
    ...
    def write(self, val):
        self.cs.write(False)
        self.mcp.write_register(self.port, self.register, [(val << 6)])
        self.cs.write(True)
...
#cs_expander_gpio.write(False)
mcp.write_register(Port.PORTB, Register.IODIR, [0b10111111])
#cs_expander_gpio.write(True)
...
#A0 = CUSTOM_GPIO_A0(mcp, cs_expander_gpio, Port.PORTA, Register.GPIO, "out")
A0 = GPIO('/dev/gpiochip1', 10, 'out')

NOT_RESET = CUSTOM_GPIO_RESET(mcp, cs_expander_gpio, Port.PORTB, Register.GPIO, "out")
...
driver = NHDC12864A1ZFSWFBWHTT(spi=SPI('/dev/spidev0.0', 3, 1e5), a0_pin=A0, reset_pin=NOT_RESET)
driver._configure()
driver.clear_screen()
